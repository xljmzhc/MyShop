...
export default {
		...
		init(id,type) {
			...
			this.$http({
				...
				if (data && data.code === 0) {
					...
					if(((json.xuexiaomingcheng!=''&&json.xuexiaomingcheng) || json.xuexiaomingcheng==0) && this.$storage.get("role")!="管理员"){
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.shangpinxiangqing = this.ruleForm.shangpinxiangqing.replace(reg,'../../../springboot2440b7ti/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
